...
# FR guidance hyperparameters. The softmax temperature (beta_t) is NOT set here -- it's
# derived from the noise schedule itself (sigma_t^2) at each guidance step, see beta_t_fn
# below. T_STAR_FRAC/T_END_FRAC are fractions of the sampling loop elapsed (0=start,
# 1=end), matching guided_reverse_loop's progress_lo/progress_hi directly. T_STAR_FRAC=0.6,
# T_END_FRAC=1.0 (default) => guidance in the last 40% of sampling (the highest-noise-
# removed, most memorization-prone regime).
...
# ---------------------------------------------------------------------------
# Flat-vector packing: FR guidance (fr_guidance.py) is model-agnostic and only
# ever sees a single [B, D] tensor. Pack/unpack translate the EGNN's (x, h)
# pair to and from that flat view (D = MAX_ATOMS*3 + MAX_ATOMS*n_types),
# zero-padding non-existent atoms exactly as the training set already is.
# ---------------------------------------------------------------------------
...
def build_training_index():
    Y_flat = pack(X_train * M_train.unsqueeze(-1), H_train * M_train.unsqueeze(-1))
    return build_flat_index(Y_flat)
...
train_index = build_training_index()
print(f"built flat training index: {train_index.shape}", flush=True)


@torch.no_grad()
def fr_guided_sample(net, B, n_steps, node_mask, lam, rng_np):
    dt = config["T"] / n_steps
    ...
    def base_step(z_t, step_idx):
        # step_idx runs 1..n_steps as "steps completed"; the EDM ancestral step
        # below uses k = remaining-steps-before-this-step, i.e. k = n_steps - step_idx
        k = n_steps - step_idx
        x_t, h_t = unpack(z_t)
    ...
    def y_fn(z_t):
        # step_idx isn't known to y_fn's signature in fr_guidance's generic driver,
        # so recompute t_vec for the *current* step from a closure-captured index.
    ...
    def step_scale_fn(step_idx):
        ...
        # injects as noise variance, NOT the cumulative marginal (1 - alpha_bar(t)).
        k = n_steps - step_idx
        t_vec = torch.full((B,), (k + 1) * dt, device=config["device"])
        s_vec = torch.full((B,), k * dt, device=config["device"])
    ...
    def beta_t_fn():
        # beta_t = sigma_t * sigma_t^T = sigma_t^2 at the CURRENT guidance step, not a
        # fixed hyperparameter -- see the note in knn_fr_guidance.fisher_rao_energy.
        return 1 - alpha_bar(y_fn.t_vec)

    energy_fn = make_knn_fr_energy_fn(train_index, K_NN, beta_t_fn)
...
for lam in LAMBDA_LIST:
    ...
    t0 = time.time()
    x_gen, types_gen, mask_gen, I_log = fr_guided_sample(net, N_SAMPLES_PER_LAMBDA, N_STEPS_EVAL, node_mask, lam, rng_np)
    ...
    out_path = os.path.join(OUT_DIR, f"samples_fr_lambda{lam:g}.pt")
    torch.save({

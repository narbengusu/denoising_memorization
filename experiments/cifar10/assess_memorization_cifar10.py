...
Memorization: distance to a training point is plain pixel-space L2 (CIFAR-10 images are
canonically oriented) -- torch.cdist against the exact N_TRAIN-image finetuning subset
(`checkpoints/train_idx.npy`). d1/d2 (nearest/second-nearest) gives the memorized-ratio
classification. This model denoises RGB pixels directly (no VAE/latent step), so raw
pixel-space L2 is sensitive to small shifts/color jitter -- worth eyeballing
`checkpoints/nearest_neighbor_grid.png` rather than trusting the ratio alone.

Fidelity: FID against the CIFAR-10 held-out test set via InceptionV3 pool features.
"""
import os
import numpy as np
import torch
...
def select_diverse_indices(order, nn_idx, n_show):
    """Keep the first n_show from `order` whose nearest-training-image hasn't been shown
    yet, so the grid doesn't repeat the same few heavily-memorized training images."""
    seen_targets = set()
    picked = []
    for i in order:
        idx = int(nn_idx[i])
...
def compute_fid(gen_01, real_01, device):
    """gen_01, real_01: [N, 3, 32, 32] in [0, 1]. Returns FID or None if the pretrained
    Inception weights aren't available."""
    ...
    def features(x):
        # move to device before normalizing (mean/std tensors are built on-device)
...
if __name__ == "__main__":
    ...
    # frac_memorized = share of samples that are copies; frac_train_memorized = share of
    # the training set they actually cover (samples can collapse onto the same few images)
    n_train = len(X_train)
    ...
    # three grids over the memorization spectrum by d1: most-memorized / borderline / least
    n_show = 8
